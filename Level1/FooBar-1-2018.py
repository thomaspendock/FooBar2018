def answer(s, i=1):
    sub_len = len(s)//i
    return sub_len if s[:i]*sub_len == s else answer(s, i=i+1)
...
Lambda hates waste and will not tolerate any leftovers, so you also want to make
sure you can serve the entire cake.
