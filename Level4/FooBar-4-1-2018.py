def incr(indexes,num):
    i = len(indexes)-1
    indexes[i] += 1
    if indexes[i] == num:
        tail = incr(indexes[:i], num-1)
        return tail + [tail[-1]+1]
    return indexes

def solution(num, req, key=0):
    bunnies = [[] for i in range(num)]
    indexes = [i  for i in range(num - req + 1)]
    while True:
        for i in indexes:
            bunnies[i].append(key)
        if len(bunnies[0]) == len(bunnies[-1]):
            return bunnies
        indexes = incr(indexes, num)
        key += 1
...
must be all of the keys. The scheme may require multiple copies of one key given
to different minions.
...
you were able to steal the keys while you were working as Commander Lambda's
assistant. The problem is, you don't know which keys to use at which consoles.
The consoles are programmed to know which keys each minion had, to prevent
someone from just stealing all of the keys and using them blindly. There are
signs by the consoles saying how many minions had some keys for the set of
...
0 and 9 (both inclusive).  For example, if you had 3 bunnies and required only
1 of them to open the cell, you would give each bunny the same key such that any
